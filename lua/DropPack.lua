...
function AlignDroppack( coords, techId, targetEntityId )
    
    local targetEntity = Shared.GetEntity( targetEntityId )
    if targetEntity and targetEntity:isa("Marine") then
        coords.origin = targetEntity:GetOrigin()
    end
