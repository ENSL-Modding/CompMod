...
function GUIAnimatedScript:Initialize(noItemsUpdateInterval)
    ...
    -- You must pass in an updateInterval to GUIAnimatedScript:Initialize when scripts
...
function GUIAnimatedScript:Reset()
    ...
    for _, item in ipairs(self.guiItems) do
        item:SetUniformScale(self.scale)
    end
    
end

function GUIAnimatedScript:OnResolutionChanged(_, _, _, newY)
...
function GUIAnimatedScript:Update(deltaTime)
    ...
    for _, completedAnim in ipairs(animationCompleteEvents) do
    ...
    for _, completedItem in ipairs(completedItems) do
