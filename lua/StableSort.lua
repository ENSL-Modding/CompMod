...
local function begin_sort( array, low, high, goes_before )
    
    local sort_count = high - low + 1
    
    if sort_count < max_chunk_size then
        insertion_sort(array, low, high, goes_before)
    else
        local workspace = {}
        --  Allocate some room.
        workspace[ math.floor( (sort_count+1)/2 ) ] = array[1]
        merge_sort( array, workspace, low, high, goes_before )
    end
...
function table.stable_sort( array, goes_before, startIndex, endIndex )
    ...
    goes_before = goes_before or default_sort
    
    local low = startIndex or 1
    local high = endIndex or #array
    begin_sort(array, low, high, goes_before)
