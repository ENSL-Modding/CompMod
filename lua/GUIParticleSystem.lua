...
function GUIParticleSystem:AddParticleType(typeName, particleValues)
    ...
    local funcList = {}
    for funcName, _ in pairs(particleValues) do
        table.insert(funcList, funcName)
    end
    self.particleTypes[typeName].funcList = funcList
...
function GUIParticleSystem:AddEmitter(emitterDefTable)
    ...
    table.insert(self.emitters, emitterDefTable.Name)
...
function GUIParticleSystem:AddModifier(modifierDefTable)
    ...
    table.insert(self.modifiers, modifierDefTable.Name)
...
function GUIParticleSystem:Update(deltaTime)
    ...
    for _, eName in ipairs(self.emitters) do
        local emitter = self.emitters[eName]
    ...
    for _, particle in ipairs(self.particles) do
        ...
        if particle.Active then
            ...
            for _, mName in ipairs(self.modifiers) do
                local modifier = self.modifiers[mName]
...
function GUIParticleSystem:_EmitParticle(emitter)
    ...
    if table.icount(emitter.particleTypesToEmit) > 0 then
        ...
        for _, funcName in ipairs(randomParticleType.funcList) do

            local values = randomParticleType[funcName]
            ...
            -- If the random value is a table, unpack the values (in this case coords) from it.
            if type(randomValue) == "table" then
                itemOperation(freeParticle.Item, GUIUnpackCoords(randomValue))
            else
...
function GUIParticleSystem:_UpdateParticle(particle, deltaTime)
    ...
    for _, lifeTimeFunc in ipairs(particle.LifeTimeFuncs) do
