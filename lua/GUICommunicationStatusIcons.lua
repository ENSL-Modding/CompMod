...
function GUICommunicationStatusIcons:Uninitialize()
    ...
    for _, icon in ipairs(self.statusIcons) do
...
function GUICommunicationStatusIcons:SetIsVisible(state)
...
function GUICommunicationStatusIcons:GetIsVisible()
...
local function UpdateAddRemoveIcons(self)
    ...
    if (self.timeOfLastCommStatusUpdate == nil) or (Shared.GetTime() > self.timeOfLastCommStatusUpdate + 0.3) then
        ...
        for _, player in ipairs(GetEntitiesForTeamWithinRange("Player", localPlayer:GetTeamNumber(), localPlayer:GetEyePos(), GUICommunicationStatusIcons.kDisplayRange)) do
...
local function UpdateIconsFromPlayers(self)
    ...
    for _, icon in ipairs(self.statusIcons) do
        ...
        local visible = self.visible and (toTargetNorm:DotProduct(viewZAxis) > 0)

        if visible then

            local worldPoint = player:GetEngagementPoint()
            local iconCoordinates
            ...
            local screenPos = Client.WorldToScreen(iconCoordinates)
