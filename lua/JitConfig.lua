...
elseif Client then
    ...
    JitConfig.defaultMaxtrace = 25000
    JitConfig.defaultMaxmcode = 40000
    JitConfig.resetSettingInBuild = 323
    ...
    JitConfig.Get = function()
        ...
        local maxtrace, maxmcode = Client.GetOptionInteger("system/jit/maxtrace", JitConfig.defaultMaxtrace), Client.GetOptionInteger("system/jit/maxmcode", JitConfig.defaultMaxmcode)
        
        -- Reset values if needed
        local lastReset = Client.GetOptionInteger("system/jit/resetInBuild", 0) 
        if lastReset < JitConfig.resetSettingInBuild and JitConfig.resetSettingInBuild <= Shared.GetBuildNumber() then
            
            local oldMaxtrace, oldMaxmcode = maxtrace, maxmcode
            maxtrace, maxmcode = JitConfig.defaultMaxtrace, JitConfig.defaultMaxmcode
            
            JitConfig.Save( maxtrace, maxmcode )
            Log("INFO: LuaJIT settings adjusted in build %s: maxtrace %s(%s), maxmcode %s(%s)", JitConfig.resetSettingInBuild, maxtrace, oldMaxtrace, maxmcode, oldMaxmcode)            
            Client.SetOptionInteger( "system/jit/resetInBuild", JitConfig.resetSettingInBuild )
...
local function UpdateConfig()
    ...
    if Shared.GetSystemTime() - lastUpdateTime >= kUpdateRate then
        ...
        local oldMaxtrace,oldMaxmcode = JitConfig.Get() 
