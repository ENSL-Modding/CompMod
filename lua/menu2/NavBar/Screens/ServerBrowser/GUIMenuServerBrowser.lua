...
local function CreateTopLeftButtons(self)
    ...
    self.refreshButton = CreateGUIObject("refreshButton", GUIMenuSimpleTextButton, self.topLeftGroupLayout,
    {
        defaultColor = MenuStyle.kOptionHeadingColor,
    })
    self.refreshButton:SetLabel(Locale.ResolveString("SERVERBROWSER_REFRESH"))
    self.refreshButton:SetFont(MenuStyle.kHeadingFont)
    self.refreshButton:AlignLeft()
    self:HookEvent(self.refreshButton, "OnPressed", self.RefreshServerList)
    
    self.startServerButton = CreateGUIObject("startServerButton", GUIMenuSimpleTextButton, self.topLeftGroupLayout,
    {
        defaultColor = MenuStyle.kOptionHeadingColor,
    })
    self.startServerButton:SetLabel(Locale.ResolveString("START_SERVER"))
    self.startServerButton:SetFont(MenuStyle.kHeadingFont)
    self.startServerButton:AlignLeft()
    self:HookEvent(self.startServerButton, "OnPressed", function()
...
local function ModDetailsCallback(modId, title, description)
    ...
    assert(pendingModNames[modId] ~= nil)
    pendingModNames[modId] = nil
    
    local modNameLookup = self.modNameLookup
    assert(modNameLookup[modId] == nil)
    modNameLookup[modId] = title
    
    self:_OnModsNameLookupUpdated()
    
end

-- Requests the mod name if not already requested.
local function RequestModName(self, modIdConverted)
    
    if self.pendingModNames[modIdConverted] == nil then
        self.pendingModNames[modIdConverted] = true
        
        Client.GetModDetails(modIdConverted, ModDetailsCallback)
    end
...
local function SetEntryModsListFromModsString(self, entry, modsString)
    ...
    for i=1, #modIds do
        ...
        local modIdConverted = tonumber(string.format("0x%s", modIds[i]))
        
        local modName = self.modNameLookup[modIdConverted]
        if modName == nil then
            ...
            RequestModName(self, modIdConverted)
...
local function UpdateServerInfoFromClientData(self, serverEntry, serverIndex)
    ...
    serverEntry:SetGameMode(FormatGameMode(Client.GetServerGameMode(serverIndex), maxPlayers))
    ...
    local modsString = Client.GetServerKeyValue(serverIndex, "mods")
    SetEntryModsListFromModsString(self, serverEntry, modsString)
...
local function OnWhiteListChanged(self)
    
    local whiteList = self:GetWhiteList()
    
    -- Update ranked status of server entries.
    for i=1, #self.serverEntries do
        local entry = self.serverEntries[i]
        entry:SetRanked(whiteList[entry:GetAddress()] or false)
    end
...
local function UpdateGameModList(self)
...
function GUIMenuServerBrowser:Initialize(params, errorDepth)
    ...
    -- Create dimmer and spinning graphic for when server list is being refreshed..
    self.dimmer = CreateGUIObject("dimmer", GUIObject, self.innerBack)
    self.dimmer:SetColor(0, 0, 0, 0)
    self.dimmer:SetVisible(false)
    self.dimmer:SetLayer(100)
    self.dimmer:SetSize(self.innerBack:GetSize())
    
    self.refreshArrows = CreateGUIObject("refreshArrows", GUIObject, self.dimmer)
    self.refreshArrows:AlignCenter()
    self.refreshArrows:SetTexture(kRefreshingArrowsTexture)
    self.refreshArrows:SetSizeFromTexture()
    self.refreshArrows:SetColor(1, 1, 1, 1)
    self.refreshArrows:SetRotationOffset(0.5, 0.5)
    self:HookEvent(self.dimmer, "OnAnimationFinished", OnDimmerFadeOutFinished)
    ...
    -- Request an up-to-date server whitelist from Hive, and in the meantime, load a cached version
    -- from disk.
    LoadWhiteList(self)
    self:HookEvent(self, "OnWhiteListChanged", OnWhiteListChanged)
...
local function UpdateRefreshingGraphics(self)
    ...
    local prevRefreshing = self.prevRefreshing == true
    
    if refreshing == prevRefreshing then
        return
    end
    
    self.prevRefreshing = refreshing
    
    -- Fade the dimmer in or out depending on if we're refreshing, or just finished refreshing.
    local dimmerColor = refreshing and kDimmerColor or Color(0, 0, 0, 0)
    local arrowColor = refreshing and Color(1, 1, 1, 1) or Color(1, 1, 1, 0)
    
    local dimmerCurrentColor = self.dimmer:GetColor()
    self.dimmer:ClearPropertyAnimations("Color")
    self.dimmer:SetColor(dimmerCurrentColor)
    self.dimmer:AnimateProperty("Color", dimmerColor, MenuAnimations.Fade, "dimmerFade"..(refreshing and "" or "Out"))
    
    local arrowsCurrentColor = self.refreshArrows:GetColor()
    self.refreshArrows:ClearPropertyAnimations("Color")
    self.refreshArrows:SetColor(arrowsCurrentColor)
    self.refreshArrows:AnimateProperty("Color", arrowColor, MenuAnimations.Fade)
    
    if refreshing then
        -- Start arrow spinning animation since it is only just now being shown.
        self.refreshArrows:AnimateProperty("Angle", nil, kArrowAnimation, "arrowRotation")
        self.dimmer:SetVisible(true)
        self.dimmer:ListenForCursorInteractions()
        self.dimmer:ListenForWheelInteractions()
    else
        self.dimmer:StopListeningForCursorInteractions()
        self.dimmer:StopListeningForWheelInteractions()
    end
