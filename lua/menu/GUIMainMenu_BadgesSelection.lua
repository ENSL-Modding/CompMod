...
function GUIBadgesSelection:LoadBadges()
    ...
    for i, availableBadge in ipairs(self.availableBadges) do
        availableBadge:Uninitialize()
        self.availableBadges[i]= nil
    end
    ...
    for _, badge in ipairs(gBadges) do
        ...
        if badgeid > gBadges.none and ownedBadges[badgeid] and not selectedbadges[badgeid] then
            ...
            self.availableBadges[i] = CreateMenuElement(self.availableRow, "Image")
            self.availableBadges[i]:SetCSSClass("badge")
            local badgeData = Badges_GetBadgeData(badgeid)
            local badgeTexture = badgeData.unitStatusTexture
            self.availableBadges[i].badgeId = badgeid
            self.availableBadges[i].badgeData = badgeData
            self.availableBadges[i].columns = ownedBadges[badgeid]
            self.availableBadges[i]:SetLeftOffset(GUIScale(5) + (i % 10 + 1) * GUIScale(36))
            self.availableBadges[i]:SetBackgroundTexture(badgeTexture)
            self.availableBadges[i]:SetIsVisible(math.ceil(i/10) == self.availableBadges.index)

            local badgeNameTooltip = GetGUIManager():CreateGUIScriptSingle("menu/GUIHoverTooltip")
            self.availableBadges[i]:AddEventCallbacks({
                OnMouseIn = function()
                    badgeNameTooltip:SetText(GetBadgeFormalName(ToString(badgeData.name)))
                    badgeNameTooltip:Show()
                end,
                OnMouseOut = function()
                    badgeNameTooltip:Hide()
                end
            })

            local activeBadges = self.activeBadges
            self.availableBadges[i].OnMouseDown = function(self)
            ...
            self.availableBadges[i].OnMouseUp = function(self)
                local columns = Badges_GetBadgeColumns(self.columns)
            ...
            self.availableBadges[i].OnMouseOver = function(self)
    ...
    if maxrow > 1 then
        local main = self
        self.previousBadges = CreateMenuElement(self.availableRow, "Image")
        ...
        self.previousBadges:AddEventCallbacks({
            OnClick = function(self)
                main.availableBadges.index = main.availableBadges.index - 1
                if main.availableBadges.index == 0 then
                    main.availableBadges.index = maxrow
                end
                main:LoadBadges()
            end
        })

        self.nextBadges = CreateMenuElement(self.availableRow, "Image")
        ...
        self.nextBadges:AddEventCallbacks({
            OnClick = function(self)
                main.availableBadges.index = main.availableBadges.index + 1
                if main.availableBadges.index > maxrow then
                    main.availableBadges.index = 1
                end
                main:LoadBadges()
            end
        })
        ...
        for i, badge in ipairs(self.availableBadges) do
...
function GUIBadgesSelection:Initialize()
    ...
    self.availableRow = CreateMenuElement(self, "Image")
    self.availableRow:SetCSSClass("badgerow2")
