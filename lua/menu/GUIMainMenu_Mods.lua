...
function GUIMainMenu:CreateModsWindow()
    ...
    back:AddEventCallbacks(
        {
            OnClick = 
                function() 
                    self.modsWindow:SetIsVisible(false) 
                    sorted = false 
                    self.modsListRefreshed = false
                end 
        }
    )
...
function GUIMainMenu:UpdateModsWindow()
    ...
    for clientModId = 1, Client.GetNumMods() do
        ...
        if state == "none" then
        --Skip any returned Mods with "None" state, as they may have been fetched from Server Browser pulling mod-details
        --No locally downloaded nor subscribed mods will have this particular state.
            goto continue
        end
