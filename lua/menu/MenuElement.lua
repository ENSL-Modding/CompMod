...
function MenuElement:HasCSSClass(className)
    return self.cssClassNameTable and table.icontains(self.cssClassNameTable, className)
end
...
--Returns false only if on of the OnShow callback didn't succeeded and returned false as well
...
--Returns false only if on of the OnHide callback didn't succeeded and returned false as well
...
function MenuElement:OnHidePost()
    for _, callback in ipairs(self.hidePostCallbacks) do
...
function MenuElement:AddEventCallbacks(callbackMapping)
    ...
    if callbackMapping.OnMouseIn then
        table.insertunique(self.mouseInCallbacks, callbackMapping.OnMouseIn, 1)
    end
    
    if callbackMapping.OnMouseOver then
        table.insertunique(self.mouseOverCallbacks, callbackMapping.OnMouseOver, 1)
    end
    
    if callbackMapping.OnMouseOut then
        table.insertunique(self.mouseOutCallbacks, callbackMapping.OnMouseOut, 1)
    end
    
    if callbackMapping.OnMouseWheel then
        table.insertunique(self.mouseWheelCallbacks, callbackMapping.OnMouseWheel, 1)
    end
    
    if callbackMapping.OnClick then
        table.insertunique(self.clickCallbacks, callbackMapping.OnClick, 1)
    end
    
    if callbackMapping.OnMouseDown then
        table.insertunique(self.mouseDownCallbacks, callbackMapping.OnMouseDown, 1)
    end
    
    if callbackMapping.OnMouseUp then
        table.insertunique(self.mouseUpCallbacks, callbackMapping.OnMouseUp, 1)
    end
    
    if callbackMapping.OnEscape then
        table.insertunique(self.escapeCallbacks, callbackMapping.OnEscape, 1)
    end
    
    if callbackMapping.OnEnter then
        table.insertunique(self.enterCallbacks, callbackMapping.OnEnter, 1)
    end
    
    if callbackMapping.OnTab then
        table.insertunique(self.tabCallbacks, callbackMapping.OnTab, 1)
    end

    if callbackMapping.OnShow then
        table.insertunique(self.showCallbacks, callbackMapping.OnShow, 1)
    end
    
    if callbackMapping.OnHide then
        table.insertunique(self.hideCallbacks, callbackMapping.OnHide, 1)
    end   
    
    if callbackMapping.OnHidePost then
        table.insertunique(self.hidePostCallbacks, callbackMapping.OnHidePost, 1)
    end   
    
    if callbackMapping.OnScrollUp then
        table.insertunique(self.scrollUpCallbacks, callbackMapping.OnScrollUp, 1)
    end
  
    if callbackMapping.OnScrollDown then
        table.insertunique(self.scrollDownCallbacks, callbackMapping.OnScrollDown, 1)
    end   
    
    if callbackMapping.OnFocus then
        table.insertunique(self.focusCallbacks, callbackMapping.OnFocus, 1)
    end
    
    if callbackMapping.OnBlur then
        table.insertunique(self.blurCallbacks, callbackMapping.OnBlur, 1)
    end
...
function MenuElement:SetBackgroundTexture(fileName)
    ...
    self.background:SetTextureCoordinates(GUIUnpackCoords(self.texCoords))
...
function MenuElement:SetTextureCoords(texCoords, time, animateFunc, animName, callBack)
    ...
    local x1, y1, x2, y2 = GUIUnpackCoords(texCoords)
...
function MenuElement:SetTexturePixelCoords(texPixelCoords, time, animateFunc, animName, callBack)
    ...
    local x1, y1, x2, y2 = GUIUnpackCoords(texPixelCoords)
