...
function LoadCSSFile(fileName)
    ...
    local fname = "game://" .. fileName

    if not GetFileExists(fname) then
        Shared.Message(string.format("Warning: CSS file %s could not be found!", fileName))
        return
    end

    local cssFile = io.open(fname, "r")
    local cssString = cssFile:read("*all")
    io.close(cssFile)
    
    --convert old format into new
    cssString = string.gsub(cssString, "css = %[%[", "", 1)
    cssString = string.gsub(cssString, "]]", "")
    ...
    local nextIndex = 1
    for i = 1, string.len(cssString) do
    
        if string.len(cssString) <= 1 then
            break
        end

        local currentReference, nextIndex = GetClassNames(cssString)
        cssString = string.sub(cssString, nextIndex, string.len(cssString))
        
        local attributes, nextIndex = GetAttributes(cssString)
        cssString = string.sub(cssString, nextIndex, string.len(cssString))
