local kMetabolizeHealthRegain = CompMod:GetLocalVariable(Metabolize.OnTag, "kMetabolizeHealthRegain", false)
local kMetabolizeEnergyRegain = CompMod:GetLocalVariable(Metabolize.OnTag, "kMetabolizeEnergyRegain", false)
    ...
    if tagName == "metabolize" and not self:GetHasAttackDelay() then
        ...
        if player then
            ...
            if player:GetCanMetabolizeHealth() then
                local totalHealed = player:AddHealth(kMetabolizeHealthRegain, false, false, false, nil, kMetabArmorHealScalar)
