...
function GUICommanderTooltip:Initialize()
    ...
    self.supplyIcon:SetTexturePixelCoordinates(GUIUnpackCoords(GUICommanderTooltip.WorkerIconCoords))
    ...
    self.biomassIcon:SetTexturePixelCoordinates(GUIUnpackCoords(GetTextureCoordinatesForIcon(kTechId.Biomass)))
    ...
    self.requiresInfo:SetTextClipped(true, xClippedSize, -1)
    ...
    self.enablesInfo:SetTextClipped(true, xClippedSize, -1)
    ...
    self.info:SetTextClipped(true, xClippedSize, -1)
...
function GUICommanderTooltip:Uninitialize()
    ...
    -- inform registered scripts
    for i = 1, #self.registeredScripts do
        local script = self.registeredScripts[i]

        if script.OnTooltipDestroy then
            script:OnTooltipDestroy()
        end
    end
