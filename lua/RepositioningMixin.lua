...
local kDefaultExtents = Vector(0.50, 0.50, 0.50)
...
function RepositioningMixin:__initmixin()
    ...
    PROFILE("RepositioningMixin:__initmixin")
...
function RepositioningMixin:_AdjustRepositioningHeight()
    ...
    local trace = Shared.TraceRay(startPointGround, endPointGround, CollisionRep.Move, PhysicsMask.AIMovement, EntityFilterMixinAndSelf(self, "Repositioning"))
    
    if trace.entity == nil or not trace.entity:isa("TechPoint") then
        
        local traceGround = Shared.TraceRay(startPointGround, endPointGround, CollisionRep.Move, PhysicsMask.AIMovement, EntityFilterMixinAndSelf(self, "Repositioning"))
...
function RepositioningMixin:FindBetterPosition(yaw, baseYaw, calls)
    ...
    local randRads  = random() * math.pi * 2
    local direction = Vector(math.cos(randRads), 0, math.sin(randRads))
    local endPoint = startPoint + direction * 5
    
    local trace = Shared.TraceRay(startPoint, endPoint, CollisionRep.Move, PhysicsMask.AIMovement, EntityFilterMixinAndSelf(self, "Repositioning"))
    
    local validPos = false
    
    if trace.fraction ~= 1 then
        endPoint = trace.endPoint + direction * -0.5
    end
    
    if (endPoint - startPoint):GetLength() >= self:GetRepositioningDistance() then
        endPoint = startPoint + direction * self:GetRepositioningDistance()
        validPos = true
    end
...
function RepositioningMixin:PerformRepositioning(deltaTime)
    ...
    if self.targetPos ~= nil then
        ...
        if direction:GetLength() < kToleranzDistance then
            ...
            -- Update pathing location if needed
            if HasMixin(self, "Obstacle") and self:GetObstacleId() ~= -1 then
                self:RemoveFromMesh()
                self:AddToMesh()
            end
