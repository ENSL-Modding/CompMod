...
PrecacheAsset("ui/order_background.dds")
...
function GUIWaypoints:Initialize()
    ...
    self.animatedCircle:SetTexturePixelCoordinates(GUIUnpackCoords(kCircleBorderTexCoords))
...
local function FindClosestWorldArrow(self, toPoint)
        ...
        for p = 1, #self.pathPoints do
            ...
            if arrowDist >= 5 then
                ...
                if trace.fraction ~= 1 then
                    ...
                    local arrow = FindClosestWorldArrow(self, arrowOrigin)
...
local function AnimateOrderChanged(self, type)
    ...
    self.orderIcon:SetTexturePixelCoordinates(GUIUnpackCoords(GetTextureCoordinatesForIcon(type)))
    TriggerCircleInAnimation(self)

end

function GUIWaypoints:OnResolutionChanged()
    UpdateItemsGUIScale(self)
...
local function AnimateFinalWaypoint(self)
    ...
    if finalWaypointData then
        ...
        self.finalDistanceText:SetIsVisible(self.visible and fullHUD)
        self.finalNameText:SetIsVisible(self.visible and fullHUD)
        ...
        self.waypointDirection:SetIsVisible(self.visible and showArrow == true)
...
function GUIWaypoints:GetShouldUpdate()
    return true
end
