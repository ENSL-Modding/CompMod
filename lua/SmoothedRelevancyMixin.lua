...
if Server then
    ...
    -- Smoothed relevancy will ordinarily end on its own after a set amount of time, but sometimes
    -- we need to ensure it will end immediately so as not to interfere with other calls to
    -- Entity:ConfigureRelevancy().
    function SmoothedRelevancyMixin:EndSmoothedRelevancy()
        self.smoothedRelevancyStart = nil
        self:ConfigureRelevancy(Vector.origin, 0)
    end
