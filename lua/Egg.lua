...
function Egg:PickUpgrades(newPlayer)
    ...
    if not Egg.kStructures then self.BuildUpgradeStructureTable() end
    local kUpgrades = Egg.kUpgrades
    local kStructures = Egg.kStructures
    ...
    for i = 1, #kStructures do
        ...
        if not picked[techId] then
            ...
            table.insert(lastUpgradeList, upgrade)
        end
    end

    newPlayer.autopickedUpgrades = true
    newPlayer.lastUpgradeList = newPlayer.lastUpgradeList or {}
    newPlayer.lastUpgradeList["Skulk"] = lastUpgradeList
end
