...
local function CommanderAbilityUpdate(self)
    ...
    if Server and (lifeSpanEnded or self:GetAbilityEndConditionsMet() or abilityType == CommanderAbility.kType.OverTime or abilityType == CommanderAbility.kType.Instant) then
        ...
        if self.OnAbilityOptionalEnd then
            self:OnAbilityOptionalEnd()
        end
...
function CommanderAbility:OnInitialized()
    ...
    if Server then
        ...
        self:SetOwner(GetCommanderForTeam(self:GetTeamNumber()))
    end
...
--All Comm/Kham abilities don't grant LOS by default
function CommanderAbility:OverrideCheckVision()
    return false
end
...
function CommanderAbility:CreateRepeatEffect() --FIXME Refactor to utilize EffectsManager
...
function CommanderAbility:DestroyRepeatEffect() --FIXME Refactor to utilize EffectsManager
...
function CommanderAbility:CreateStartEffect() --FIXME Refactor to utilize EffectsManager
...
function CommanderAbility:CreateEndEffect() --FIXME Refactor to utilize EffectsManager
