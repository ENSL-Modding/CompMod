...
function PowerPoint:CanBeCompletedByScriptActor( player )
    ...
    if player:isa("Marine") then
        -- Player can always build if they have double-tapped their flashlight
        local playerHasConfirmed, playerFlashlightToggleTime = player:GetFlashlightToggled()
        playerHasConfirmed = playerHasConfirmed and Shared.GetTime() < playerFlashlightToggleTime + 1.5 -- Can only be completed within a certain amount of time after flashing
        if playerHasConfirmed then
            return true
        end
    end
...
function PowerPoint:AllowConstructionComplete( player )
    return self:CanBeCompletedByScriptActor( player )
end
...
function PowerPoint:GetCanConstructOverride( player )
    ...
    return isBuildable and ( self.buildFraction < 1 or self:CanBeCompletedByScriptActor(player) )
end
...
function PowerPoint:OverrideCheckVision()
    return self.powerState == PowerPoint.kPowerState.socketed and self:GetIsBuilt() and self:GetHealth() > 0 and self:GetAttackTime() > 0
end
...
if Server then
    ...
    function PowerPoint:OnConstruct( builder, buildFraction, oldBuildFraction )
        for _,thresh in ipairs( kPrimeSoundThresholds ) do
            if oldBuildFraction < thresh and thresh <= buildFraction then
                StartSoundEffectOnEntity( kAlmostReady, self, kAlmostReadyVolume, nil )
                break
            end
        end
    end
