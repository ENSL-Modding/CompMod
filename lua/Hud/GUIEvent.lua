...
function GUIEvent:Initialize()
    ...
    self.middleBorder:SetTexturePixelCoordinates(GUIUnpackCoords(GUIEvent.kMiddleTextureCoords))
    ...
    self.middleBackground:SetTexturePixelCoordinates(GUIUnpackCoords(GUIEvent.kMiddleTextureCoords))
...
function GUIEvent:Reset(scale)
    ...
    self.unlockDescription:SetFontName(GUIEvent.kFontName)
    GUIMakeFontScale(self.unlockDescription)
    self.unlockBottomText:SetFontName(GUIEvent.kFontName)
    GUIMakeFontScale(self.unlockBottomText)
...
function GUIEvent:Update(_, parameters)
    ...
    if notification ~= nil then
        ...
        if table.icount(self.displayedNotifications) > 0 and self.displayedNotifications[1]:MatchesTo(notification.LocationName, notification.TechId) then
    ...
    local currentNotificationCount = table.icount(self.displayedNotifications)
    if self.lastNotificationCount ~= currentNotificationCount then
