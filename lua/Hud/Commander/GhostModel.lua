...
function GhostModel:Update()
    ...
    if self.loadedModelIndex ~= modelIndex then
        ...
        self.renderModel:SetModel(modelIndex)   --TODO Deal with Webs
        ...
        if player and player.GetIgnoreGhostHighlight and player:GetIgnoreGhostHighlight() ~= true then
            self.renderModel:SetMaterialParameter("highlight", 0.96)

            --FIXME This is shit, but no fast way around this as entire GhostModel code paths are stuctured, yay crap hacks. Sincerely, McG.
            if player:isa("Gorge") then
                if player:GetVariant() == kGorgeVariant.toxin then --FIXME This doesn't cover Clogs (sep model)
                    self.renderModel:SetMaterialParameter("textureIndex", 1)
                end
            end
    ...
    if not modelCoords or not modelCoords:GetIsFinite() then
        ...
        if player and player.currentTechId then
            ...
            if radius then
                ...
                local origin = Vector(modelCoords.origin)
                if type(radius) == "table" then
                    for i = 1, #radius do
                        player:AddGhostGuide(origin, radius[i])
                    end
                else
                    player:AddGhostGuide(origin, radius)
                end
