...
local function CreateCostDisplay()
    ...
    resIcon:SetTexturePixelCoordinates(GUIUnpackCoords(kResourceIconTextureCoordinates))
...
local function UpdateCystModels(self, cystPoints, nbExistingCystUsed, existingCyst)

    if self.loadedModelIndex then
    
        local numCurrentCystModel = #self.cystModels
        local numDesiredCystModels = math.max(0, #cystPoints - nbExistingCystUsed)
        ...
        for i = 1, #cystPoints do
            ...
            local model = self.cystModels[modelIndex]
            
            -- Don't overlap the model of an existing cyst
            if existingCyst[i] == nil and point and model then
...
function CystGhostModel:Update()
    ...
    local parent
    local normals = {}
    local existingCyst = nil
    local nbExistingCystUsed = 0
    ...
    if modelCoords then        
        ...
        cystPoints, parent, normals, nbExistingCystUsed, existingCyst = GetCystPoints(modelCoords.origin, true, player:GetTeamNumber())
        ...
        if self.costDisplay then
            ...
            local cost = (#cystPoints - nbExistingCystUsed) * kCystCost
        ...
        if not existingCyst[#cystPoints] then
            ...
            self.renderModel:SetIsVisible(false)
        end
    ...
    UpdateCystModels(self, cystPoints, nbExistingCystUsed, existingCyst)
