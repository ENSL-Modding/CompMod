...
function GUIFlamethrowerDisplay:Initialize()
    ...
    self.clipDisplay:SetTexturePixelCoordinates(GUIUnpackCoords(kBarTexCoords))
    ...
    for i = 1, kNumAmmo do
        ...
        ammoIcon:SetTexturePixelCoordinates(GUIUnpackCoords(kAmmoTexCoords))
...
function GUIFlamethrowerDisplay:SetWeaponVariant(weaponVariant)
    local texturePath
    if weaponVariant == 1 then -- vanilla skin
        texturePath = "ui/FlamethrowerDisplay.dds"
    else -- extra skin
        texturePath = string.format("ui/FlamethrowerDisplay%d.dds", weaponVariant-1)
    end
    
    self.background:SetTexture(texturePath)
    self.clipDisplay:SetTexture(texturePath)
    for i=1, #self.ammoIcons do
        self.ammoIcons[i]:SetTexture(texturePath)
    end
end
