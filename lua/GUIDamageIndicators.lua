...
GUIDamageIndicators.kMinIndicatorSize = 0.25
...
function GUIDamageIndicators:Update(deltaTime)
    ...
    local numDamageIndicators = table.icount(damageIndicators) / 2
    
    if numDamageIndicators ~= table.icount(self.indicatorItems) then
    ...
    for i, indicatorItem in ipairs(self.indicatorItems) do
        ...
        local currentSize = math.max(math.pow(currentAlpha, 16), GUIDamageIndicators.kMinIndicatorSize)
...
function GUIDamageIndicators:OnTakeDamage(position, rotation, hitType)
    ...
    hitEffect:SetTextureCoordinates(GUIUnpackCoords(pixelCoords))
...
function GUIDamageIndicators:ResizeIndicatorList(numIndicators)
    ...
    while numIndicators > table.icount(self.indicatorItems) do
...
function GUIDamageIndicators:CreateIndicatorItem()
    ...
    if table.icount(self.reuseItems) > 0 then
