...
function GUIPing:Initialize()
    ...
    self:SetIsVisible(true) -- init to true, but won't be visible until used.
    self.pingItem.Frame:SetIsVisible(false) -- frame is not visible
    self.expiredPingTime = 0 -- init to 0 so it isn't shown at the beginning.
    
end

function GUIPing:SetIsVisible(state)
...
function GUIPing:Update(deltaTime)
    ...
    for index, entity in ientitylist(Shared.GetEntitiesWithClassname("TeamInfo")) do
        ...
        if pingTime ~= self.expiredPingTime then
            ...
            self.pingItem.Frame:SetIsVisible(self.visible and (timeSincePing <= kCommanderPingDuration))
