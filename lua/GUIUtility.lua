...
function GUIScale(size)
    return math.min(Client.GetScreenWidth(), Client.GetScreenHeight()) / kScreenScaleAspect * size
end
...
function GUICreateButtonIcon(forAction, alienStyle)
    ...
    if override then
        ...
        keyBackground:SetTexturePixelCoordinates(GUIUnpackCoords(override.coords))
...
function GUIMakeFontScale(item, itemFontFamily, itemSize)
    ...
    if itemFontFamily and itemSize then
        ...
        if fontName then
            local FontFamilies = FontFamilies
            for _, familyName in ipairs(FontFamilies) do
                local family = FontFamilies[familyName]
...
function GUIMixColor(dst, src, scalar)
