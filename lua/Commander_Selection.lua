...
function Commander:GetEntitiesBetweenVecs(potentialEntities, pickStartVec, pickEndVec, entityList)
    ...
    for _, entity in ipairs(potentialEntities) do
...
local function FilterOutMarqueeSelection(selection)
    ...
    local hasMoveableStructure = false
    local toRemove = unique_set()
    ...
    for _, entity in ipairs(selection) do
        ...
        if entity:isa("Commander") then
            ...
            if entity.GetIsMoveable and entity:GetIsMoveable() then
                ...
                foundStructure = true
                hasMoveableStructure = hasMoveableStructure or HasMixin(entity, "Pathing")
            end
    ...
    elseif foundStructure and not foundNonStructure and hasMoveableStructure then

        for _, entity in ipairs(selection) do
        
            -- filter out non moveables structures (mostly aliens)
            if not HasMixin(entity, "Pathing") then
                toRemove:Insert(entity)
            end
    ...
    for _, entity in ipairs(toRemove:GetList()) do
