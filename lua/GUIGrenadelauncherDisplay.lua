...
local kTextures = 
{
    ...
    "models/marine/grenadelauncher/grenade_launcher_view_display1.dds",
}
...
function GUIGrenadelauncherDisplay:Initialize()
    ...
    self.viewPitch:SetTexturePixelCoordinates(GUIUnpackCoords(kViewPitchTexCoords))
    
    self.background = GUIManager:CreateGraphicItem()
    self.background:SetSize( Vector(128, 256, 0) )
    self.background:SetPosition( Vector(128, 0, 0))
    self.background:SetTexturePixelCoordinates(GUIUnpackCoords(kBgTexCoords))
    ...
    self.ammoDisplay:SetScale(Vector(0.5, 0.5, 0) * Vector(1.18, 1.18,0))
    ...
    for i = 1, kNumGrenades do
        ...
        grenadeIcon:SetTexturePixelCoordinates(GUIUnpackCoords(kGrenadeBlueTexCoords))
...
function GUIGrenadelauncherDisplay:Update(deltaTime)
    ...
    if prevVariant ~= weaponVariant then
        ...
        local texturePath = kTextures[weaponVariant]
        self.viewPitch:SetTexture(texturePath)
        self.background:SetTexture(texturePath)
        for i=1, kNumGrenades do
            self.grenadeIcons[i]:SetTexture(texturePath)
        end
    end
    ...
    for i = 1, kNumGrenades do
        ...
        if i == kNumGrenades then
            ...
            if self.weaponClip == 1 then
                grenadeIcon:SetTexturePixelCoordinates(GUIUnpackCoords(kGrenadeRedTexCoords))
            else
                grenadeIcon:SetTexturePixelCoordinates(GUIUnpackCoords(kGrenadeBlueTexCoords))
            end
