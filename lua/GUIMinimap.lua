...
function GUIMinimap:UpdateItemsGUIScale()
    ...
    for _, c in ipairs(self.nameTagList) do
        local v = self.nameTagMap[c]
        GUI.DestroyItem(v)
    end
...
function GUIMinimap:Initialize()
    ...
    for k,_ in ipairs(kMinimapActivity) do
...
function GUIMinimap:InitializeLocationNames()
    ...
    for _, name in ipairs(multipleLocationsData) do
        ...
        table.insert(uniqueLocationsData, { Name = name, Origin = averageOrigin / table.icount(origins) })
...
local function GetFreeNameTag(self, clientIndex)
    ...
    local nameTag
    for i, c in ipairs(self.nameTagList) do
        local v = self.nameTagMap[c]
        if now - v.lastUsed > kNameTagReuseTimeout then
            nameTag = v
            table.remove(self.nameTagList, i)
    ...
    if not self.nameTagMap[clientIndex] then
        table.insert(self.nameTagList, clientIndex)
    end
...
function GUIMinimap:UpdateBlipActivity()
    ...
    for _, entity in ientitylist(Shared.GetEntitiesWithTag("MinimapMappable")) do
        ...
        if icon and icon.entity ~= entity then
            ...
            table.removevalue(self.iconList, id)
            self.iconMap[id] = nil
            icon.entity = nil
            FreeIcon(self, icon)
        end
        if not self.iconMap[id] then
            table.insert(self.iconList, id)
        ...
        if activity then
            ...
            if self.updateRadius > 0 then
                local diff = (self.playerOrigin - entity:GetMapBlipOrigin()) * Vector(1,0,1)
    ...
    for i, id in ipairs(self.iconList) do
        local icon = self.iconMap[id]
...
local function GetFreeDynamicBlip(self, xPos, yPos, blipType)
    ...
    local returnBlip
    if table.icount(self.reuseDynamicBlips) > 0 then
...
local function AddLocalBlip(self, key, position, blipType, blipTeam)
    ...
    table.insert(self.localBlipKeys, key)
...
local function RemoveLocalBlip(self, key)
    ...
    if blip then
        ...
        table.removevalue(self.localBlipKeys, key)
    end
end
...
function GUIMinimap:DrawLocalBlips()
    ...
    for _, key in ipairs(self.localBlipKeys) do
        local blipData = self.localBlipData[key]
...
local function UpdateDynamicBlips(self)
    ...
    local numBlips = table.icount(newDynamicBlips) / blipItemCount
...
function GUIMinimap:ResetAll()
    self.resetAll = true
    for _, id in ipairs(self.iconList) do
        local icon = self.iconMap[id]
        icon.resetMinimapItem = true
    end
end
