...
-- If the length of the text before the message uses up more than this fraction
-- of the permitted width, force the message onto a new line.
local kForceNewLineFraction = 0.9
...
function GUIChat:Initialize()
    ...
    self.visible = true -- or rather, is this ALLOWED to be visible...
...
function GUIChat:SetIsVisible(state)
...
function GUIChat:Update(deltaTime)
    ...
    for i, message in ipairs(self.messages) do
        ...
        -- only show messages if the GUIScript is visible.
        message["Background"]:SetIsVisible(self.visible)
    ...
    if ChatUI_EnteringChatMessage() then
        ...
        if not self.inputItem:GetIsVisible() then
            ...
            self.inputModeItem:SetIsVisible(self.visible)
            self.inputItem:SetIsVisible(self.visible)
