...
if Client then
    ...
    local gPlayAmbientSounds = true

    local function OnToggleAmbientSounds()
        if Shared.GetCheatsEnabled() or Shared.GetTestsEnabled() then
            gPlayAmbientSounds = not gPlayAmbientSounds
            
            if not gPlayAmbientSounds then
                for a = 1, #Client.ambientSoundList do
                    Client.ambientSoundList[a]:OnDestroy()
                end
                Client.ambientSoundList = {}
                Print("Muting Ambient Sounds")

            elseif gPlayAmbientSounds then
                for _, ambient in pairs(Client.cachedAmbientSoundList) do
    ...
    Event.Hook("Console_togglemapsounds", OnToggleAmbientSounds)
