...
function GUIFeedbackState_Reason:CreateButton(topOffset, text, id)
    ...
    buttontext:SetFontName(GUIGameFeedback.kFont)
    buttontext:SetColor(GUIGameFeedback.kTextColor)
...
class 'GUIGameFeedback' (GUIAnimatedScript)

GUIGameFeedback.kFont = Fonts.kAgencyFB_Large_Bold

GUIGameFeedback.kTextColor = kMarineFontColor
GUIGameFeedback.kActiveTextColor = kAlienFontColor

GUIGameFeedback.kBackgroundTexture = PrecacheAssetSafe("ui/feedback/background.dds")
GUIGameFeedback.kFeedbackIconTexture = PrecacheAssetSafe("ui/feedback/feedback_icon.dds")

function GUIGameFeedback:GetIsVisible()
    return self.isVisible
end

function GUIGameFeedback:OnResolutionChanged(oldX, oldY, newX, newY)
...
function GUIGameFeedback:Initialize()
...
function GUIGameFeedback:ResetState()
    self:SetIsVisible(false)

    self:SetState(_G.GUIFeedbackState_Rating)
end

function GUIGameFeedback:Close()
    self.rating = -1
...
function GUIGameFeedback:SetIsVisible(visible)
...
function GUIGameFeedback:SetState(state)
...
function GUIGameFeedback:GetState()
    return self.state and self.state.parent.state --Todo: find out why this is needed
end

function GUIGameFeedback:SendKeyEvent(key, down)
...
GUIGameFeedback.kMinPlayTime =  3 * 60 --3 minutes

function GUIGameFeedback:Update(deltaTime)
    PROFILE("GUIGameFeedback:Update")
...
function GUIGameFeedback:SetWaitLevel(level)
    self.waitLevel = level

    self:ResetWaitTime()
end

function GUIGameFeedback:SetWaitTime(time)
    self.waitTime = time
end

function GUIGameFeedback:GetWaitLevel()
    return self.waitLevel
end

function GUIGameFeedback:GetWaitTime(time)
...
function GUIGameFeedback:ResetWaitTime()
...
function GUIGameFeedback:Uninitialize()
...
function GUIGameFeedback:_InitializeBackground()
    ...
    self.question:SetFontName(GUIGameFeedback.kFont)
    ...
    self.question:SetColor(GUIGameFeedback.kTextColor)
    self.content:AddChild(self.question)

end

function GUIGameFeedback:_UninitializeBackground()
