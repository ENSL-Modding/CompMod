...
local useNearbyLight1 = true

Event.Hook("Console_unl", function() 
    useNearbyLight1 = not useNearbyLight1
    Log("unl = %s", useNearbyLight1)
end)
...
function Player:UpdateDirtFalling(_)
...
function Player:UpdateOnosRumble(_)
...
function Player:ResetShakingLights()
    ...
    if self.shakingLightList then
        ...
        for _, renderLight in ipairs(self.shakingLightList) do
            renderLight.lightShakeTime = 0
        end
