...
function GUICommanderAlerts:OnResolutionChanged(oldX, oldY, newX, newY)
    ...
    local numberMessages = table.icount(self.messages)
...
function GUICommanderAlerts:Uninitialize()
    ...
    for _, message in ipairs(self.reuseMessages) do
        GUI.DestroyItem(message["Background"])
    end
    self.reuseMessages = { }
...
local validMarineAlerts = { kTechId.MarineAlertNeedAmmo, kTechId.MarineAlertNeedMedpack, kTechId.MarineAlertNeedOrder, kTechId.MarineAlertNeedStructure, kTechId.MarineAlertSoldierUnderAttack }
...
function GUICommanderAlerts:Update(deltaTime)
    ...
    local numberMessages = #addAlertMessages / numberElementsPerMessage
    ...
    for _, removeMessage in ipairs(removeMessages) do
