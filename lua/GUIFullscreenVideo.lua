...
local oldSoundVolume, oldMusicVolume, oldVoiceVolume
...
function GUIFullscreenVideo:Initialize()
    ...
    local offsetX, offsetY, sizeX, sizeY = CalculatePositioning()
...
function GUIFullscreenVideo:PlayVideo(videoFile, videoLength, onEndFunc)
    ...
    Client.SetVideoVolume(oldSoundVolume)
...
function GUIFullscreenVideo:OnResolutionChanged()
    self:Uninitialize()
    self:Initialize()
end
...
function GUIFullscreenVideo:End()
    ...
    if self.audioFileName then
        Shared.StopSound(nil, self.audioFileName)
    end
...
function GUIFullscreenVideo:SetAudioFileName(fileName)
    self.audioFileName = fileName
end


function GUIFullscreenVideo:Update(_)
    ...
    elseif self.state == "loadingHtml" then
        if self.videoWebView:GetUrlLoaded() then
            ...
            if self.audioFileName then
                Shared.PlaySound(nil, self.audioFileName)
            end
        end
...
function GUIFullscreenVideo:SendKeyEvent(key, _)
