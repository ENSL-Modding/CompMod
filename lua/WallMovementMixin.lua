...
function WallMovementMixin:GetAverageWallWalkingNormal(extraRange, feelerSize)
    ...
    if not normalFound then
        for i=0, 7 do
            ...
            normalFound = self:TraceWallNormal(startPoint, startPoint + Vector(math.cos(theta) * wallWalkingRange * 0.707, wallWalkingRange * 0.707, math.sin(theta) * wallWalkingRange * 0.707), wallNormals, feelerSize)
            if normalFound then
                break
            end
        end
