...
local function GetTechIdsWithPrereq(prereqTechId)
    ...
    if techTree then
        ...
        for _, techId in ipairs(techTree.techIdList) do
...
local function UpdateAbilityList(self, currentBioMassLevel, bioMassAlertLevel, alertColor)
    ...
    for i = 1, #kBioMassTechIds do
        ...
        for j, techId in ipairs(GetTechIdsWithPrereq(kBioMassTechIds[i])) do
            ...
            levelIcon.Graphic:SetTexturePixelCoordinates(GUIUnpackCoords(GetTextureCoordinatesForIcon(techId)))
...
function GUIBioMassDisplay:Initialize()
    ...
    self.background:SetTexturePixelCoordinates(GUIUnpackCoords(kBackgroundCoords))
    self.background:SetLayer(kGUILayerPlayerHUD)
    self.background:SetIsVisible(false) -- prevent 1-frame pop-in
...
function GUIBioMassDisplay:Update(deltaTime)
    ...
    --[[
    local rate = showGUI and 15 or -15
    ...
    --]]
    self.background:SetIsVisible(showGUI) --FIXME This handled HiveUI conflict, but need to revist transp & update rate
    self.smokeyBackground:SetIsVisible(showGUI)
...
function GUIBioMassDisplay:OnTooltipDestroy()
    self.registered = false
end
