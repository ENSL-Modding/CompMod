local function ApplyDamage(self, targetList)
    ...
    for index, targetEntry in ipairs(targetList) do
        ...
        if entity and (self.dotMarkerType == self.kType.SingleTarget and self.targetId == entity:GetId() or self.targetIds[entity:GetId()]) and entity:GetCanTakeDamage() and (not self.immuneCondition or not self.immuneCondition(self, entity, targetEntry.damage * self.damageIntervall)) then
